...
export default async function Home() {
  ...
  const recentTransactions = recentTransactionsData.map(t => ({
    ...
    status: t.status === 'IN_PROGRESS' ? 'In Progress' : t.status === 'COMPLETED' ? 'Completed' : 'Returned',
  }));
          ...
          <div className="bg-white shadow-sm rounded-2xl overflow-hidden border border-red-100">
            <div className="px-6 py-4 border-b border-red-100 bg-red-50/50 flex items-center justify-between">
              <div className="flex items-center">
                ...
                <h2 className="text-lg font-bold text-gray-900 uppercase tracking-wider">Active Handouts</h2>
              </div>
            ...
            <div className="p-0">
              {inProgressTransactions.length === 0 ? (
                <div className="p-8 text-center text-gray-400">
                  <p className="italic text-sm">No active handouts in progress.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50/50">
                      <tr>
                        <th className="px-6 py-3 text-left text-xs font-black text-gray-500 uppercase tracking-widest">Cadet</th>
                      ...
                      {inProgressTransactions.map((t) => (
                        <tr key={t.id} className="hover:bg-red-50/30 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-bold text-gray-900">{t.user.name}</div>
                            <div className="text-[10px] font-bold text-gray-400 uppercase">
                              {t.status === 'IN_PROGRESS' ? 'Issue' : 'Return'} • {t.checkoutDate.toLocaleDateString()}
                            </div>
                          </td>
              ...
              <ul className="space-y-4">
                {recentTransactions.map((t) => (
                  <li key={t.id} className="relative pl-4 border-l-2 border-red-700">
                    ...
                    <p className="text-[10px] font-bold text-gray-500 uppercase">{t.user} • {t.date}</p>
                    <span className={`inline-block mt-1 text-[8px] font-black uppercase px-1.5 py-0.5 rounded ${
                      t.status === 'In Progress' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
                    }`}>
                      {t.status}
                    </span>
                  </li>
